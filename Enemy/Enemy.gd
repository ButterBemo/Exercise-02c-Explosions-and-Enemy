...
var intitial_position = Vector2.ZERO
...
func _ready():
	intitial_position.x = -100
	intitial_position.y = y_positions[randi() % y_positions.size()]
	position = intitial_position
	
func _physics_process(_delta):
	position += direction
	position.y = intitial_position.y + sin(position.x/20)*wobble
	if position.x >= 1200:
		queue_free()
